...
class GuitarChord {
	...
	rootToneFilter(preResult) {
		...
		preResult.forEach((item) => {
			...
			// 从低音弦到高音弦遍历，不符合最低音条件则禁止其发声
			for (var i = item.length - 1; i >= 0; i--) {
				if (item[i].key !== this.bassTone) {
					item[i].fret = null;
					item[i].key = null;
					realStringLength--;
				} else {
	...
	// 和弦指法计算入口（支持转位和弦）
	chord() {
		let chordTone, inversion = 0;
		if (is(arguments[0])('Array')) {
			chordTone = arguments[0];
			inversion = arguments[1] || 0;
		} else {
		...
		// 处理转位和弦
		let originalChordTone = chordTone.slice();
		if (inversion > 0 && inversion < chordTone.length) {
			// 转位：将指定的音移到最前面作为最低音
			let bassNote = chordTone[inversion];
			chordTone = [bassNote].concat(chordTone.filter((note, index) => index !== inversion));
		}
		
		// 和弦组成音
		this.chordTone = chordTone;
		// 原始根音（用于和弦命名）
		this.rootTone = originalChordTone[0];
		// 最低音（用于指法过滤）
		this.bassTone = chordTone[0];
		// 转位级别
		this.inversion = inversion;
		// 原始和弦音序列（用于和弦命名）
		this.originalChordTone = originalChordTone;
...
class ChordName {
	...
	// 计算和弦名（支持转位和弦）
	getChordName(chordTone, originalChordTone = null, inversion = 0) {
		// 如果是转位和弦，使用原始和弦组成音来判断和弦类型
		let analysisChordTone = originalChordTone || chordTone;
		let rootKey = analysisChordTone[0];
		// 和弦的字母名
		...
		if (analysisChordTone.length === 3) {
			suffixArr = chord3SuffixMap.filter((item) => {
				return item.fn.bind(this, analysisChordTone)();
			});
		...
		// 拼接基本和弦名
		let baseName = chordRootName + suffix;
		
		// 如果是转位和弦，添加转位标记
		if (inversion > 0 && originalChordTone && inversion < originalChordTone.length) {
			let bassKey = chordTone[0]; // 当前最低音
			let bassName = this.getKeyName(bassKey);
			return baseName + '/' + bassName;
		}
...
class ChordSvg {
	 ...
	 * @param originalChordTone 原始和弦组成音（用于转位和弦）
	 * @param inversion 转位级别
	 */
	drawChord(chordTone, chord, target, originalChordTone = null, inversion = 0) {
		...
		// 在图案上侧绘制和弦名称（支持转位和弦）
		this.setChordName(svg, chordName.getChordName(chordTone, originalChordTone, inversion));
		// 将生成号的svg图案塞到指定结构中
